...
def get_tagger_cats(corpus, label_binarizers=None):
    ...
    s_ret_xpos = ['N/A'] * len(corpus)
    s_ret_xpost_left_1 = ['N/A'] * len(corpus)
    s_ret_xpost_left_2 = ['N/A'] * len(corpus)
    s_ret_xpost_right_1 = ['N/A'] * len(corpus)
    s_ret_xpost_right_2 = ['N/A'] * len(corpus)
    s_ret_upos = ['N/A'] * len(corpus)
    s_ret_upost_left_1 = ['N/A'] * len(corpus)
    s_ret_upost_left_2 = ['N/A'] * len(corpus)
    s_ret_upost_right_1 = ['N/A'] * len(corpus)
    s_ret_upost_right_2 = ['N/A'] * len(corpus)
    s_ret_dep = ['N/A'] * len(corpus)
    s_ret_dep_siblings = [[] for _ in range(len(corpus))]
    s_ret_dep_children = [[] for _ in range(len(corpus))]
    s_ret_dep_parent = ['N/A'] * len(corpus)
    s_ret_feats = [[] for _ in range(len(corpus))]
    s_ret_ner = ['O'] * len(corpus)
    ...
    for i in tqdm(range(len(corpus))):
        ...
        for dic in sents:
            for token in dic:
                f = 0
                if token['text'].lower() == corpus[i][3].strip().lower().split(' ')[0]:
                    ...
                    ret_ner[i] = token['ner']
                    f += 1
                elif token['text'].lower() == corpus[i][3].strip().lower().split(' ')[1]:
                    s_ret_xpos[i] = token['xpos']
                    s_ret_xpost_left_1[i] = dic[token['id'] - 1]['xpos'] if token['id'] > 0 else 'N/A'
                    s_ret_xpost_left_2[i] = dic[token['id'] - 2]['xpos'] if token['id'] > 1 else 'N/A'
                    s_ret_xpost_right_1[i] = dic[token['id'] - 1]['xpos'] if token['id'] < len(dic) else 'N/A'
                    s_ret_xpost_right_2[i] = dic[token['id'] - 2]['xpos'] if token['id'] < len(dic) - 1 else 'N/A'
                    s_ret_upos[i] = token['upos']
                    s_ret_upost_left_1[i] = dic[token['id'] - 1]['upos'] if token['id'] > 0 else 'N/A'
                    s_ret_upost_left_2[i] = dic[token['id'] - 2]['upos'] if token['id'] > 1 else 'N/A'
                    s_ret_upost_right_1[i] = dic[token['id'] - 1]['upos'] if token['id'] < len(dic) else 'N/A'
                    s_ret_upost_right_2[i] = dic[token['id'] - 2]['upos'] if token['id'] < len(dic) - 1 else 'N/A'
                    s_ret_dep[i] = token['deprel']
                    s_ret_dep_siblings[i] = [t['deprel'] for t in dic if ('head' in token and 'head' in t and t['head'] == token['head'])]
                    s_ret_dep_children[i] = [t['deprel'] for t in dic if ('head' in t and t['head'] == token['id'])]
                    s_ret_dep_parent[i] = dic[token['head']]['deprel'] if ('head' in token and token['head'] > 0 and token['head'] < len(dic)) else 'N/A'
                    s_ret_feats[i] = token['feats'].split('|') if 'feats' in 'feats' in token else 'N/A'
                    s_ret_ner[i] = token['ner']
    ...
    s_xpos_binarizer = LabelBinarizer()
    s_xpost_left_1_binarizer = LabelBinarizer()
    s_xpost_left_2_binarizer = LabelBinarizer()
    s_xpost_right_1_binarizer = LabelBinarizer()
    s_xpost_right_2_binarizer = LabelBinarizer()
    s_upos_binarizer = LabelBinarizer()
    s_upost_left_1_binarizer = LabelBinarizer()
    s_upost_left_2_binarizer = LabelBinarizer()
    s_upost_right_1_binarizer = LabelBinarizer()
    s_upost_right_2_binarizer = LabelBinarizer()
    s_dep_binarizer = LabelBinarizer()
    s_dep_siblings_binarizer = MultiLabelBinarizer()
    s_dep_parent_binarizer = MultiLabelBinarizer()
    s_dep_children_binarizer = MultiLabelBinarizer()
    s_feats_binarizer = MultiLabelBinarizer()
    s_ner_binarizer = LabelBinarizer()
    ...
    s_xpos_transformed = None
    s_xpost_left_1_transformed = None
    s_xpost_left_2_transformed = None
    s_xpost_right_1_transformed = None
    s_xpost_right_2_transformed = None
    s_upos_transformed = None
    s_upost_left_1_transformed = None
    s_upost_left_2_transformed = None
    s_upost_right_1_transformed = None
    s_upost_right_2_transformed = None
    s_dep_transformed = None
    s_dep_siblings_transformed = None
    s_dep_children_transformed = None
    s_dep_parent_transformed = None
    s_feats_transformed = None
    s_ner_transformed = None
    ...
    if label_binarizers != None:
        ...
        s_xpos_binarizer = label_binarizers[16]
        s_xpost_left_1_binarizer = label_binarizers[17]
        s_xpost_left_2_binarizer = label_binarizers[18]
        s_xpost_right_1_binarizer = label_binarizers[19]
        s_xpost_right_2_binarizer = label_binarizers[20]
        s_upos_binarizer = label_binarizers[21]
        s_upost_left_1_binarizer = label_binarizers[22]
        s_upost_left_2_binarizer = label_binarizers[23]
        s_upost_right_1_binarizer = label_binarizers[24]
        s_upost_right_2_binarizer = label_binarizers[25]
        s_dep_siblings_binarizer = label_binarizers[26]
        s_dep_children_binarizer = label_binarizers[27]
        s_dep_binarizer = label_binarizers[28]
        s_dep_parent_binarizer = label_binarizers[29]
        s_feats_binarizer = label_binarizers[30]
        s_ner_binarizer = label_binarizers[31]
        ...
        s_xpos_transformed = s_xpos_binarizer.transform(ret_xpos)
        s_xpost_left_1_transformed = s_xpost_left_1_binarizer.transform(ret_xpost_left_1)
        s_xpost_left_2_transformed = s_xpost_left_2_binarizer.transform(ret_xpost_left_2)
        s_xpost_right_1_transformed = s_xpost_right_1_binarizer.transform(ret_xpost_right_1)
        s_xpost_right_2_transformed = s_xpost_right_2_binarizer.transform(ret_xpost_right_2)
        s_upos_transformed = s_upos_binarizer.transform(ret_upos)
        s_upost_left_1_transformed = s_upost_left_1_binarizer.transform(ret_upost_left_1)
        s_upost_left_2_transformed = s_upost_left_2_binarizer.transform(ret_upost_left_2)
        s_upost_right_1_transformed = s_upost_right_1_binarizer.transform(ret_upost_right_1)
        s_upost_right_2_transformed = s_upost_right_2_binarizer.transform(ret_upost_right_2)
        s_dep_transformed = s_dep_binarizer.transform(ret_dep)
        s_dep_parent_transformed = s_dep_parent_binarizer.transform(ret_dep_parent)
        s_dep_siblings_transformed = s_dep_siblings_binarizer.transform(ret_dep_siblings)
        s_dep_children_transformed = s_dep_children_binarizer.transform(ret_dep_children)
        s_feats_transformed = s_feats_binarizer.transform(ret_feats)
        s_ner_transformed = s_ner_binarizer.transform(ret_ner)
    else:
        ...
        s_xpos_transformed = s_xpos_binarizer.fit_transform(ret_xpos)
        s_xpost_left_1_transformed = s_xpost_left_1_binarizer.fit_transform(ret_xpost_left_1)
        s_xpost_left_2_transformed = s_xpost_left_2_binarizer.fit_transform(ret_xpost_left_2)
        s_xpost_right_1_transformed = s_xpost_right_1_binarizer.fit_transform(ret_xpost_right_1)
        s_xpost_right_2_transformed = s_xpost_right_2_binarizer.fit_transform(ret_xpost_right_2)
        s_upos_transformed = s_upos_binarizer.fit_transform(ret_upos)
        s_upost_left_1_transformed = s_upost_left_1_binarizer.fit_transform(ret_upost_left_1)
        s_upost_left_2_transformed = s_upost_left_2_binarizer.fit_transform(ret_upost_left_2)
        s_upost_right_1_transformed = s_upost_right_1_binarizer.fit_transform(ret_upost_right_1)
        s_upost_right_2_transformed = s_upost_right_2_binarizer.fit_transform(ret_upost_right_2)
        s_dep_transformed = s_dep_binarizer.fit_transform(ret_dep)
        s_dep_parent_transformed = s_dep_parent_binarizer.fit_transform(ret_dep_parent)
        s_dep_siblings_transformed = s_dep_siblings_binarizer.fit_transform(ret_dep_siblings)
        s_dep_children_transformed = s_dep_children_binarizer.fit_transform(ret_dep_children)
        s_feats_transformed = s_feats_binarizer.fit_transform(ret_feats)
        s_ner_transformed = s_ner_binarizer.fit_transform(ret_ner)
    ...
    s_xpos_df = pd.DataFrame(s_xpos_transformed)
    s_xpos_left_1_df = pd.DataFrame(s_xpost_left_1_transformed)
    s_xpos_left_2_df = pd.DataFrame(s_xpost_left_2_transformed)
    s_xpos_right_1_df = pd.DataFrame(s_xpost_right_1_transformed)
    s_xpos_right_2_df = pd.DataFrame(s_xpost_right_2_transformed)
    s_upos_df = pd.DataFrame(s_upos_transformed)
    s_upos_left_1_df = pd.DataFrame(s_upost_left_1_transformed)
    s_upos_left_2_df = pd.DataFrame(s_upost_left_2_transformed)
    s_upos_right_1_df = pd.DataFrame(s_upost_right_1_transformed)
    s_upos_right_2_df = pd.DataFrame(s_upost_right_2_transformed)
    s_dep_df = pd.DataFrame(s_dep_transformed)
    s_dep_parent_df = pd.DataFrame(s_dep_parent_transformed)
    s_dep_children_df = pd.DataFrame(s_dep_children_transformed)
    s_dep_siblings_df = pd.DataFrame(s_dep_siblings_transformed)
    s_feats_df = pd.DataFrame(s_feats_transformed)
    s_ner_df = pd.DataFrame(s_ner_transformed)
    ...
    return ret, (
        xpos_binarizer, 
        xpost_left_1_binarizer, 
        xpost_left_2_binarizer, 
        xpost_right_1_binarizer, 
        xpost_right_2_binarizer, 
        upos_binarizer, 
        upost_left_1_binarizer, 
        upost_left_2_binarizer, 
        upost_right_1_binarizer, 
        upost_right_2_binarizer, 
        dep_siblings_binarizer, 
        dep_children_binarizer, 
        dep_binarizer, 
        dep_parent_binarizer, 
        feats_binarizer, 
        ner_binarizer,
        s_xpos_binarizer, 
        s_xpost_left_1_binarizer, 
        s_xpost_left_2_binarizer, 
        s_xpost_right_1_binarizer, 
        s_xpost_right_2_binarizer, 
        s_upos_binarizer, 
        s_upost_left_1_binarizer, 
        s_upost_left_2_binarizer, 
        s_upost_right_1_binarizer, 
        s_upost_right_2_binarizer, 
        s_dep_siblings_binarizer, 
        s_dep_children_binarizer, 
        s_dep_binarizer, 
        s_dep_parent_binarizer, 
        s_feats_binarizer, 
        s_ner_binarizer   
    )
...
def get_wordnet_cats(corpus):
    ...
    s_nums_hypernyms = [0.0] * len(corpus)
    s_nums_hyponyms = [0.0] * len(corpus)
    s_nums_member_holonyms = [0.0] * len(corpus)
    s_nums_part_meronyms = [0.0] * len(corpus)
    s_nums_member_meronyms = [0.0] * len(corpus)
    s_hypernym_paths = [0.0] * len(corpus)
    s_nums_examples = [0.0] * len(corpus)
    s_nums_root_hypernyms = [0.0] * len(corpus)

    for i in tqdm(range(len(corpus))):
        syns = wn.synsets(corpus[i][3].strip().lower().split(' ')[0],lang='eng')
        jj = 0
        for syn in syns:
        ...
        syns = wn.synsets(corpus[i][3].strip().lower().split(' ')[1],lang='eng')
        jj = 0
        for syn in syns:
            s_nums_hypernyms[i] += len(syn.hypernyms())
            s_nums_hyponyms[i] += len(syn.hyponyms())
            s_nums_member_holonyms[i] += len(syn.member_holonyms())
            s_nums_part_meronyms[i] += len(syn.part_meronyms())
            s_nums_member_meronyms[i] += len(syn.member_meronyms())
            s_hypernym_paths[i] += min([len(path) for path in syn.hypernym_paths()])
            s_nums_examples[i] += len(syn.examples())
            s_nums_root_hypernyms[i] += len(syn.root_hypernyms())
        ...
        s_nums_hypernyms[i] /= jj
        s_nums_hyponyms[i] /= jj
        s_nums_member_holonyms[i] /= jj
        s_nums_part_meronyms[i] /= jj
        s_nums_member_meronyms[i] /= jj
        s_hypernym_paths[i] /= jj
        s_nums_examples[i] /= jj
        s_nums_root_hypernyms[i] /= jj
    ...
    return [
        ...
        pd.Series(s_nums_hypernyms, dtype="float"),
        pd.Series(s_nums_hypernyms, dtype="float"),
        pd.Series(s_nums_member_holonyms, dtype="float"),
        pd.Series(s_nums_part_meronyms, dtype="float"),
        pd.Series(s_nums_member_meronyms, dtype="float"),
        pd.Series(s_hypernym_paths, dtype="float"),
        pd.Series(s_nums_examples, dtype="float"),
        pd.Series(s_nums_root_hypernyms, dtype="float")
    ]

def get_embedding_cats(corpus, embeddings):
    dims = [np.zeros(len(corpus)) for _ in range(embeddings.embedding_length)]
    s_dims = [np.zeros(len(corpus)) for _ in range(embeddings.embedding_length)]

    for i in tqdm(range(len(corpus))):
        sentence = Sentence(corpus[i][2])
        embeddings.embed(sentence)
        for token in sentence:
            if token.text.strip().lower() == corpus[i][3].strip().lower().split(' ')[0]:
                embeddingVec = token.embedding.cpu().detach().numpy()
                for dim in range(len(embeddingVec)):
                    dims[dim][i] = embeddingVec[dim]
            elif token.text.strip().lower() == corpus[i][3].strip().lower().split(' ')[1]:
                embeddingVec = token.embedding.cpu().detach().numpy()
                for dim in range(len(embeddingVec)):
                    s_dims[dim][i] = embeddingVec[dim]

                break
    
    return [pd.Series(k) for k in dims] + [pd.Series(k) for k in s_dims]

def get_subtlex_cats(corpus, subtlex):
    dims = [np.zeros(len(corpus)) * len(corpus), [0.0] * len(corpus)]
    s_dims = [np.zeros(len(corpus)) * len(corpus), [0.0] * len(corpus)]

    for i in tqdm(range(len(corpus))):
        w = corpus[i][3].lower().split(' ')[0]
        if w not in subtlex:
            w = w[:-1]
        if w not in subtlex:
            w = w[:-1]
        ...
        if w not in subtlex:
            w = w[:-1]
        if w in subtlex:
            s_dims[0][i] = float(subtlex[w][5])
            s_dims[1][i] = float(subtlex[w][7])
...
def get_emoti_cats(corpus, emoti):
    ...
    for i in tqdm(range(len(corpus))):
        ...
        if w not in emoti:
            continue
        for e in range(13):
            s_dims[e][i] = float(emoti[w][e])
    
    return [
        pd.Series(dims[e]) for e in range(len(dims))
    ] + [
        pd.Series(s_dims[e]) for e in range(len(s_dims))
    ]

def get_affec_cats(corpus, affec):
    dims = [np.zeros(len(corpus)) for i in range(63)]
    s_dims = [np.zeros(len(corpus)) for i in range(63)]
    ...
    for i in tqdm(range(len(corpus))):
        ...
        for e in range(63):
            s_dims[e][i] = float(affec[w][e])

    return [
        pd.Series(dims[e]) for e in range(len(dims))
    ] + [
        pd.Series(s_dims[e]) for e in range(len(dims))
    ]

def get_bigrams_cats(corpus, bigrams):
    dims = np.zeros(len(corpus))
    s_dims = np.zeros(len(corpus))

    for i in tqdm(range(len(corpus))):
        av_bigrams_freq = 0.0
        word_bigrams = list(nltk.bigrams(corpus[i][3].lower().split(' ')[0]))
        cnnt = 0
        ...
        av_bigrams_freq = 0.0
        word_bigrams = list(nltk.bigrams(corpus[i][3].lower().split(' ')[1]))
        cnnt = 0
        for bi in word_bigrams:
            av_bigrams_freq += bigrams[(bi[0] + bi[1])] if (bi[0] + bi[1]) in bigrams else 0
            cnnt += 1
...
def get_efllex_cats(corpus, efllex):
    ...
    s_dims = [np.zeros(len(corpus)) for _ in range(111)]
...
#path_single_train = 'C:/semeval/train/lcp_multi_train.tsv'
#path_single_trial = 'C:/semeval/trial/lcp_multi_trial.tsv'
#path_single_test = 'C:/semeval/test/lcp_multi_test.tsv'
...
#x.to_feather('train_x_multi.feather')
#y.to_feather('train_y_multi.feather')
...
#x_e.to_feather('eval_x_multi.feather')
#y_e.to_feather('eval_y_multi.feather')
...
#x_t.to_feather('test_x_multi.feather')

#jb.dump(binarizers_o, 'binarizers_o.bin')
#jb.dump(syllable_binarizers_o, 'syllable_binarizers_o.bin')

x = pd.read_feather('train_x_multi.feather')
y = pd.read_feather('train_y_multi.feather')
x_e = pd.read_feather('eval_x_multi.feather')
y_e = pd.read_feather('eval_y_multi.feather')
x_t = pd.read_feather('test_x_multi.feather')

pool = Pool(data=x, label=y)
pool_e = Pool(data=x_e, label=y_e)
pool_t = Pool(data=x_t)
pool.quantize()
pool.save_quantization_borders("borders_multi.dat")
pool_e.quantize(input_borders="borders_multi.dat")
pool_t.quantize(input_borders="borders_multi.dat")

#grid = {'depth': [4, 5, 6, 7, 8, 9, 10],
#        'l2_leaf_reg': [1, 3, 5, 7, 9]}

catboost_model = CatBoostRegressor(verbose=True, eval_metric='R2', grow_policy='Lossguide', l2_leaf_reg=15, learning_rate=0.01, depth=6, iterations=5000, max_leaves=15, loss_function='RMSE')
catboost_model.fit(X=pool, eval_set=pool_e, use_best_model=True)
catboost_model.save_model('multi.cbm')
...
print(importances)
with open('feature_importances_multi.txt', 'w') as f:
    f.write(importances.to_string())

#out_file = read_out('random_predictions_test_multi.csv')
#to_out(corpus, out_file, y_pred)
#write_out('random_predictions_test_multi.csv', out_file)
